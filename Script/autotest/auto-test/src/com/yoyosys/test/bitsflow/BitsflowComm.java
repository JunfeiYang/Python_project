  ...
  implements ChannelListener, MessageListener, GroupCommListener, BitsFlowRPCHandler
{
  ...
  public BitsFlowMessage createC2SReadyMessage() throws Exception
  {
    logger.info("createC2SReadyMessage");
    return createMessage(SERVER_SUBJECT, C2S_READY);
  }

  public void M2SPublishHeartbeat(boolean blocking)
  {
    logger.info("M2S publish heartbeat");
    publishMessage(blocking, SERVER_SUBJECT, M2S_HEARTBEAT);
  }
  public void M2CPublishHeartbeat(boolean blocking)
  {
    logger.info("M2C publish heartbeat");
    publishMessage(blocking, CLIENT_SUBJECT, M2C_HEARTBEAT);
  }
  public void S2MPublishHeartbeat(boolean blocking)
  {
    logger.info("S2M publish heartbeat");
    publishMessage(blocking, MANAGER_SUBJECT, S2M_HEARTBEAT);
  }
  public void C2MPublishHeartbeat(boolean blocking)
  {
    logger.info("C2M publish heartbeat");
    publishMessage(blocking, MANAGER_SUBJECT, C2M_HEARTBEAT);
  }
  public void C2SPublishReady(boolean blocking, int caseID)
  {
    logger.debug("C2S publish ready-message");
    publishMessage(blocking, SERVER_SUBJECT, C2S_READY, caseID);
  }
  public void C2SPublishReport(boolean blocking, int caseID, int code, String info)
  {
    logger.info("C2S publish report");
    publishMessage(blocking, SERVER_SUBJECT, C2S_REPORT_TASK, caseID, code, info);
  }
