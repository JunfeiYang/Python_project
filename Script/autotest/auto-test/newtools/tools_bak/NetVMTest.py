...
class NetVMTest(BitsflowTest, NetVMUtil):
  ...
  def getWorkflowLog(self, pc):
    ...
    tarLogCmd = "rm -rf WorkflowLog-*.tar.gz;mkdir %s;cd %s;rm -rf *;gdb --pid=`cat %s` --command=~/yoyo_gdb_attach | tee workflow.stack;cp -rf %s %s %s %s ./;cd ../;tar cvzf %s %s" %(tarFolder, tarFolder, workflow_pid_name, workflow_log_name, os.path.join(workflow_home,"*core*"), workflow_conf_name, workflow_db_name, tarLog, tarFolder)
  ...
  def getDschedLog(self, pc):
    ...
    tarLogCmd = "rm -rf DschedLog-*.tar.gz;mkdir %s;cd %s;rm -rf *;gdb --pid=`cat %s` --command=~/yoyo_gdb_attach | tee dsched.stack;cp -rf %s %s %s %s ./;cd ../;tar cvzf %s %s" %(tarFolder, tarFolder, dsched_pid_name, dsched_log_name, os.path.join(dsched_home,"*core*"), dsched_conf_name, dsched_db_name, tarLog, tarFolder)
