...
class DataCellTest(BitsflowTest):
  ...
  def do_backupDataCellVolume(self, user_input):
    ...
    if len(user_input) == 0:
      ...
      for volume in datacell_volumes:
        cmd = "mkdir %s; mv %s %s ;  mkdir %s ; cp -R %s %s "%(user_input, volume, user_input,volume,os.path.join(datacell_storage_home),user_input)
  ...
  def do_backupDataCellLog(self, user_input):
    ...
    if len(user_input) == 0:
      ...
      cmd = "tar -zcvf  %s.tar.gz  %s ; tar -zcvf  core.tar.gz  %s ;"%(os.path.join(datacell_log_home, "%s"%datacell_log_name),os.path.join(datacell_log_home, "%s*"%datacell_log_name), os.path.join(datacell_storage_home,"*core*"));
      cmd  += "mkdir %s; mv %s.tar.gz %s; mv  core.tar.gz %s; %s > %s;"%(user_input, os.path.join(datacell_log_home, "%s"%datacell_log_name), user_input, user_input, self.createYoyopkgCmd("ls"), os.path.join(user_input, "version"))
  ...
  def do_psDataCell(self, user_input):
    """Usage: ps [processName] (If no processName, we'll "ps DataCell")"""
    if len(user_input) == 0:
      cmd = "ps aux|grep %s/bin/DataCell"%release_home
    else:
      cmd = "ps aux|grep %s/bin/%s"%(release_home,user_input)
    self.serialExecute(cmd)
