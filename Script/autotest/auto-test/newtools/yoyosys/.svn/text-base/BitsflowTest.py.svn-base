...
class BitsflowTest(Cmd, Bitsflow):
  ...
  def do_restartAgent(self, user_input):
    self.restartAgent(logLevel="agent_log_level")
  ...
  def do_getBitsflowLog(self, user_input):
    ...
    self.getBitsflowLogs(agent_list)
  ...
  def do_tar(self, user_input):
    """Usage: tar direc"""
    if len(user_input) < 1:
      print "usage : tar direct"
    else:
      cmd = "sudo ./tarjcf.sh %s"%(user_input)
      self.parallelExecute(cmd);
