...
class TitanTest(Login, Cmd):
  ...
  def rmTitanLog(self, pc):
    cmd = "rm -rf %s"%(titan_log)
    self.parallelExecute(cmd)


  def getTitanLog(self, pc):
    print "\n====== IP: " + pc + " getLog ======"
    tarFolder = "TitanLog-"+pc 
    tarLogCmd = "rm -rf TitanLog*;mkdir %s;cp %s %s;tar cvzf TitanLog.tar.gz %s"%(tarFolder, titan_log, tarFolder, tarFolder)
    self.singleExecute(pc, tarLogCmd)

  def do_getTitanLog(self, user_input):
    """Download titan logs"""
    try:
      self.startWork(self.getTitanLog, pc_list)
      self.do_getFile("TitanLog.tar.gz")
    except:
      print sys.exc_info()
    finally:
      pass
  ...
  def do_configTitan(self, user_input):
    self.putFile("titan.sh", "%s/bin/titan.sh")
    self.putFile("gremlin.sh", "%s/bin/gremlin.sh")
