...
#        sys.exit()
#########
def   login_ssh(username, hostname, port, password, cmdline):
	...
	t.load_system_host_keys()
        t.connect(username = username, hostname = hostname, port = port, password = password)
        #t.connect(username, hostname, port, password)
	...
	cmd_list = cmdline.split(";");
        for cmd in cmd_list:
         ...
         try:
          print '' 
          print '#########################################' 
          print 'Beginning to Command execution %s ' % datetime.datetime.now() 
          print 'Command execution:', stdout.read()
          print 'Command execution success %s ' % datetime.datetime.now() 
          print '' 
          print '##########################################'
         except:
          ret = Failure
...
if  __name__ == '__main__':
	...
	parser.add_option("-c", "--cmdline", dest="cmdline", default='whoami',
			  help="Remote login commands executed  "
			       "example: ./paramiko_ssh_cmd.py -u root -a"
			       " 172.16.205.28 -p 22 -P redhat -c \"whoami;date;ifconfig\"",
			  metavar="CMDLINE")
(options, args) = parser.parse_args()
print 'options: %s, args: %s' % (options, args)
ssh = login_ssh(options.username, options.hostname, options.port, options.password, options.cmdline)
